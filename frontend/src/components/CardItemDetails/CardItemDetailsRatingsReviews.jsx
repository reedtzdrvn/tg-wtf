...
const CardItemDetailsRatingsReviews = ({ reviews }) => {
  const ratingsCounts = {};
  reviews.forEach(review => {
    const rating = review.ratingsCount;
        ...
        {[5, 4, 3, 2, 1].map(rating => {
          ...
          return (
            <li key={rating} className="flex items-center mb-[8px] relative">
              <span className="mr-[5px] w-[10px] text-center">{rating}</span>
              <svg
                width="16"
                height="16"
                viewBox="0 0 35 33"
              ...
              <div className="card-item-details-ratings-rectangles-wrapper w-[40%] h-[1vh] ml-[8px]">
                <div className="card-item-details-ratings-rectangle" style={{ width: barWidth, height: '100%' }}></div>
              </div>
            </li>
          );
        })}
